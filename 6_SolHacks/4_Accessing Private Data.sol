...
Уязвимость
Все данные смарт-контракта можно прочитать.

Давайте посмотрим, как мы можем читать частные данные. 
В процессе вы узнаете, как Solidity хранит переменные состояния.
*/
...
/*
Примечание: нельзя использовать web3 на JVM, поэтому используйте контракт, развернутый на ropsten.
Примечание: браузер Web3 устарел, поэтому используйте Web3 из консоли трюфеля.

Контракт развернут на Ropsten
...
# Хранилище
- 2**256 слотов
- 32 байта на каждый слот
- данные хранятся последовательно в порядке объявления
- хранение оптимизировано для экономии места. Если соседние переменные подходят к одному
   32 байта, то они упаковываются в один и тот же слот, начиная справа
*/
...
contract Vault {
    ...
    // slot 3, 4, 5 (по одному на каждый элемент массива)
    ...
    // слот 6 - длина массива
     // начиная со слота hash(6) - элементы массива
     // слот, в котором хранится элемент массива = keccak256(slot)) + (index * elementSize)
     // где слот = 6 и elementSize = 2 (1 (uint) + 1 (bytes32))
    User[] private users;
    // слот 7 - пустой
     // записи хранятся в хеше (ключ, слот)
     // где слот = 7, ключ = ключ карты
