/*
Uniswap V2 Оптимальная односторонняя подача
Оптимальная односторонняя подача
*/
...
contract TestUniswapOptimalOneSidedSupply {
   ...
   /*
     s = оптимальная сумма свопа
     r = сумма резерва для токена a
     a = количество токенов a у пользователя в настоящее время (еще не добавлено в резерв)
     f = процент комиссии за своп
  ...
  /* Оптимальная односторонняя поставка
     1. Обмен оптимальной суммы с токена A на токен B
     2. Добавьте ликвидности
     */
    function zap(
        address _tokenA,
        address _tokenB,
        ...
        if (IUniswapV2Pair(pair).token0() == _tokenA) {
            // меняем с token0 на token1
            swapAmount = getSwapAmount(reserve0, _amountA);
        } else {
            // меняем с token1 на token0
